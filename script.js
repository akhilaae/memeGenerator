...
function clearAll() {
    const memecont=document.querySelector('.meme-content');
     memecont.innerHTML='';
    const jokecont=document.querySelector('.joke-content');
     jokecont.innerHTML='';
    const quotecont=document.querySelector('.quote-content');
     quotecont.innerHTML='';
    const riddlecont=document.querySelector('.riddle-content');
  ...
  function showMeme() {
    ...
    newelem.setAttribute('src',randomMemeUrl);
    const ismeme=memecont.querySelector('img')
    clearAll();
    memecont.appendChild(newelem);
  }
  function showJoke() {
    ...
    jokeelem.textContent=randomJokeText;
    const isjoke=jokecont.querySelector('p');
    clearAll();
    jokecont.appendChild(jokeelem);
  }
  ...
  function showQuote() {
    ...
    let str=randomQuote.quote;
    str=str.concat(" - ",randomQuote.author);
    quoteelem.textContent=str;
    const isquote=quotecont.querySelector('p');
    clearAll();
    quotecont.appendChild(quoteelem);
  }
  ...
  function showRiddle() {
    ...
    qelem.textContent=randomRiddle.question;
    const anselem=document.createElement('p');
    anselem.textContent=randomRiddle.answer;
    anselem.setAttribute('id','riddle-answer');
    anselem.hidden=true;
    const isriddle=riddlecont.querySelector('p');
    clearAll();
    riddlecont.appendChild(qelem);
    riddlecont.appendChild(anselem);
  }
function revealAnswers() {
    const riddlecont=document.querySelector('.riddle-content');
    const isriddle=riddlecont.querySelector('p');
    const isanswer=riddlecont.querySelector('#riddle-answer');
    if(isriddle && !isanswer.hidden){
        alert("Answer already revealed");
    }
    else if(isriddle && isanswer.hidden){
        isanswer.hidden=false;
    }
    else{
        alert("no riddle to answer");
    }
}  
